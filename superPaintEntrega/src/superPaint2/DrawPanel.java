...
public class DrawPanel extends JPanel
{
    ...
    private int numRealShapes; //number of shapes to draw. The other are saved in the list but not drawn. Useful for undo
    ...
    public int getNumRealShapes() {
		return numRealShapes;
	}

	public void setNumRealShapes(int numRealShapes) {
		this.numRealShapes = numRealShapes;
	}
    ...
    {
    	 if ((!myShapes.isEmpty()) && (getNumRealShapes()!=0))
        {
            setNumRealShapes(getNumRealShapes()-1);
            repaint();
        }
    }
    ...
    {
        if ((!myShapes.isEmpty()) && (getNumRealShapes()!=0) && (getNumRealShapes()<myShapes.size()))
        {
        	setNumRealShapes(getNumRealShapes()+1);
            repaint();
        }
    ...
    private class MouseHandler extends MouseAdapter 
    {
        ...
        public void mouseReleased( MouseEvent event )
        {
            ...
            //HUECO 4
            for ( int counter=numRealShapes+1; counter<myShapes.size(); counter++ )
                myShapes.remove(counter);
            numRealShapes++;            
